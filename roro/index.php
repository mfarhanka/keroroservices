...
<section id="hero" class="relative overflow-hidden bg-gradient-to-b from-blue-50 via-white to-red-50 py-12 md:py-20">
    ...
    <div class="relative mx-auto max-w-6xl px-4 text-center">
        <div class="mx-auto max-w-4xl">
            ...
            <p class="mx-auto mt-5 max-w-2xl text-base font-medium leading-7 text-gray-600">From design to delivery, Kaundar Enterprise provides value-added environmental, disposal and transport services throughout Kuala Lumpur and Selangor.</p>
        ...
        <div class="mt-10 rounded-3xl border-2 border-blue-200 bg-white p-5 text-left shadow-2xl md:p-7">
            <div class="mb-5 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
                <div><p class="text-xs font-extrabold uppercase tracking-widest text-rorored">Quick Enquiry</p><h2 class="font-heading text-xl font-black uppercase text-gray-900 md:text-2xl">Book Your Service</h2></div>
                <p class="text-xs font-semibold text-gray-500">Complete the details and continue via WhatsApp</p>
            </div>
            <form id="quick-booking-form" action="booking.php" method="post" class="grid gap-3 md:grid-cols-3">
                <label class="block text-xs font-bold uppercase text-roroblue md:col-span-3"><i class="fa-solid fa-location-dot mr-1 text-rorored"></i> Select Location
                    <select id="quick-location" name="location" size="6" required class="mt-2 w-full rounded-2xl border-2 border-blue-200 bg-blue-50 p-2 text-sm font-semibold normal-case text-gray-800 outline-none focus:border-roroblue focus:ring-2 focus:ring-blue-200">
                        <option value="">Choose one area below</option>
                        <?php foreach (['Kuala Lumpur','Dengkil','Cyberjaya','Putrajaya','Sepang / Salak Tinggi','Puchong','Bangi / Kajang','Sungai Buloh','Rawang','Nilai','Shah Alam','Subang Jaya','Banting','Jalan Ipoh','Other area in Kuala Lumpur / Selangor'] as $area): ?>
                        <option value="<?=htmlspecialchars($area)?>" class="rounded-lg px-3 py-2"><?=htmlspecialchars($area)?></option>
                        <?php endforeach; ?>
                    </select>
                    <span class="mt-2 block text-[11px] font-semibold normal-case text-gray-500">Tap one area. The selected row will be highlighted.</span>
                </label>
                <label class="block text-xs font-bold uppercase text-roroblue"><i class="fa-solid fa-dumpster mr-1 text-rorored"></i> Bin / Package
                    <select id="quick-package" name="package" required class="mt-1.5 w-full rounded-xl border border-blue-200 bg-blue-50 px-3 py-3 text-sm font-semibold normal-case text-gray-800 outline-none focus:ring-2 focus:ring-roroblue">
                        <option value="">Select package</option><option>12 x 6 x 2½ CY Bin</option><option>12 x 6 x 4 CY Bin</option><option>12 x 6 x 5 CY Bin</option><option>Other / Not sure</option>
                    </select>
                </label>
                <label class="block text-xs font-bold uppercase text-roroblue"><i class="fa-solid fa-list-check mr-1 text-rorored"></i> Service
                    <select id="quick-service" name="service" required class="mt-1.5 w-full rounded-xl border border-blue-200 bg-blue-50 px-3 py-3 text-sm font-semibold normal-case text-gray-800 outline-none focus:ring-2 focus:ring-roroblue">
                        <option value="">Select service</option><option>Wastage</option><option>Sewerage</option><option>Liquid Disposal</option><option>Recycle Items</option><option>Transport & Heavy Goods</option><option>Cutting Trees & Disposal</option><option>Clear Debris & Disposal</option>
                    </select>
                </label>
                <button type="submit" class="self-end rounded-xl bg-rorored px-4 py-3 font-heading text-sm font-black uppercase text-white shadow-lg hover:bg-rorored-dark">Book Now <i class="fa-solid fa-arrow-right ml-2"></i></button>
            </form>
            <p id="quick-booking-error" class="mt-3 hidden rounded-xl bg-red-50 px-4 py-2 text-sm font-semibold text-rorored" role="alert"></p>
        </div>

        <div class="mt-8 grid gap-3 sm:grid-cols-3">
            <div class="rounded-2xl border border-blue-100 bg-white p-4 shadow"><b class="block font-heading text-2xl text-roroblue">1994</b><span class="text-[10px] font-bold uppercase text-gray-500">Established</span></div>
            <div class="rounded-2xl border border-red-100 bg-white p-4 shadow"><b class="block font-heading text-2xl text-rorored">7</b><span class="text-[10px] font-bold uppercase text-gray-500">Core Services</span></div>
            <div class="rounded-2xl border border-blue-100 bg-white p-4 shadow"><b class="block font-heading text-2xl text-roroblue">30+</b><span class="text-[10px] font-bold uppercase text-gray-500">Years of Experience</span></div>
        </div>
    </div>
</section>
